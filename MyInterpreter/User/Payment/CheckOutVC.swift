...
class CheckOutVC: UIViewController {
    ...
    override func viewDidLayoutSubviews() {
        ...
        self.totalRow.frame = CGRect(x: insets.left, y: self.paymentRow.frame.maxY,
                                     width: width, height: rowHeight)
        self.confirmButton.frame = CGRect(x: insets.left, y: 0, width: 88, height: 44)
        self.confirmButton.center = CGPoint(x: width/2.0, y: self.totalRow.frame.maxY + rowHeight*1.5)
...
extension CheckOutVC: STPPaymentContextDelegate
{
    ...
    func paymentContext(_ paymentContext: STPPaymentContext, didFinishWith status: STPPaymentStatus, error: Error?) {
        ...
        case .success:
            title = "Success"
            message = "You booked a \(self.product)!"
